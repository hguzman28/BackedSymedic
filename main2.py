...
#data: dict,
   ...
   #Direccion y telefonos sede ppal
    ...
    return {"statusCode": 200,"resultado": "success","paciente":datapac,"grupo": list(respuesta),"respie":va_respie,"sedeppal":list(response_querysp),"membrete": va_memb}
...
def insert_data(data: Labcombo): 
    try:
        print(data)
        print(len(data.nombre))

        if len(data.nombre) < 2 or len(data.codcombo) < 2 :
              raise HTTPException(status_code=400, detail="Campo nombre y codcombo es requerido y mayor a 2") 
...
def insert_data(data: dict): 
	query='insert into laborden(fecorden,estado,codage,numorden,agefaccli,numfaccli,numfaccli,tipide,codpac,codemp,contpac,contpac,interlab,prio,codmedico,nivpac,copago,cuotam,bono,descpac,abopac,coddiagnostico,empcodi,percodi,totemp,codpac,usuario,dur,descpacp,factura,edadf,facturar,pendaprob,embar,fechasol,usuvbo,horvbo,idbono) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)' 
	sdk.insertPostgres(query, data,'prd') 
# Endpoint delete 
